...
export const getAllSavedPost = async (userId) => {
    try {
        ...
        // // // Getting all posts first then we'll filter out your saved data from all posts.
        const posts = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.videoCollectionId,
...
export const addFollow = async (byUser, toUser) => {
    ...
    const res1 = await databases.updateDocument(
        ...
        {
            followedBy: [byUser.$id, ...toUser.followedBy]
        }
    ...
    const res2 = await databases.updateDocument(
        ...
        byUser.$id,
        {
            followedBy: [toUser.$id, ...byUser.followedBy]
        }
    )
...
export const removeFollow = async (byUser, toUser) => {
    ...
    let filteredArrForByUser = byUser.followedBy.filter((ele) => {
        if (ele !== toUser.$id) {
            return ele
        }
    })
    ...
    const res2 = await databases.updateDocument(
        ...
        byUser.$id,
        {
            followedBy: filteredArrForByUser
        }
    )



    let filteredArrForToUser = toUser.followedBy.filter((ele) => {
        if (ele !== byUser.$id) {
            return ele
        }
    })
    ...
    const res1 = await databases.updateDocument(
        ...
        toUser.$id, // // // Kyoki smne wale use ko upadate krna chahata hu
        {
            followedBy: filteredArrForToUser
        }
    )
